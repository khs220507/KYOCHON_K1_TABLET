...
class SlotCard extends StatefulWidget {
  ...
  final bool shouldBlink; // 이동 예정일 때 깜박거림
...
class _SlotCardState extends State<SlotCard> {
  ...
  void didUpdateWidget(SlotCard oldWidget) {
    ...
    // 이동 예정 상태가 변경되면 깜박거림 업데이트
    if (widget.shouldBlink) {
      if (!_isBlinking) {
        _startBlinking();
      }
    } else if (!widget.basketState.isOvercooking ||
        widget.basketState.selectedMenu == null) {
      // 오버쿡이 아니고 이동 예정도 아니면 깜박거림 중지
      final totalCookTime = widget.basketState.selectedMenu?.cookTime;
  ...
  Widget build(BuildContext context) {
    ...
    // 카드 색상 결정: 사용불가 > 이동중(1번) > 곧 도착 예정(타겟) > 예약됨 > 메뉴 있음 > 비어있음
    final Color cardColor;
    final isReserved =
        widget.basketState.isWaiting ||
        widget.basketState.isArrivingSoon ||
        widget.basketState.pendingMoveTo != null;

    if (widget.basketState.isUnavailable) {
      // 사용불가 상태: 비활성화 색상 (회색)
      cardColor = const Color(0xFFBDBDBD);
    } else if (widget.basketState.isMoving && widget.slotNumber == 0) {
      // 1번 바스켓 이동 중: 하얀색으로 비어지는 느낌
      cardColor = const Color(0xFFFFFFFF);
    } else if (widget.basketState.isArrivingSoon) {
      // 타겟 바스켓 곧 도착 예정: 진한 초록색으로 채워지는 느낌
      cardColor = const Color(0xFF81C784); // 진한 초록색
    } else if (isReserved) {
      // 타겟 바스켓 (예약됨): 연한 연두색
      cardColor = const Color(0xFFDCEDC8); // 연한 연두색
    ...
    return AnimatedOpacity(
      opacity: widget.shouldBlink && _isBlinking ? 0.3 : 1.0,
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            // 오버쿡 표시 (기준 시간 전: 검은색, 기준 시간 이후: 빨간색 깜박)
            if (widget.basketState.isOvercooking)
              Padding(
                padding: EdgeInsets.only(top: 10 * widget.scale),
                child: Builder(
                  builder: (context) {
                    ...
                    final overcookLimit =
                        (totalCookTime * overcookPercent / 100).round();
                    final isOverLimit =
                        widget.basketState.overcookTime >= overcookLimit;
                    final shouldBlink = isOverLimit && _isBlinking;

                    return AnimatedOpacity(
                      opacity: shouldBlink ? 0.0 : 1.0,
                      duration: const Duration(milliseconds: 1000),
                      child: Text(
                        '오버쿡 +${_formatTime(widget.basketState.overcookTime)}',
                        style: TextStyle(
                          fontSize: 35 * widget.scale, // 조리 타이머와 같은 크기
                          fontWeight: FontWeight.bold,
                          color: isOverLimit
                              ? const Color(0xFFD32F2F) // 기준 시간 이후: 빨간색
                              : const Color(0xFF000000), // 기준 시간 전: 검은색
                        ),
                      ),
                    );
                  },
